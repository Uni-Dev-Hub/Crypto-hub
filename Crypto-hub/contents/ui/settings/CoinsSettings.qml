...
Kirigami.ScrollablePage {
    ...
    function loadModel() {
        ...
        var coins = root.cfg_favoriteCoins;
        if ((!coins || coins.trim() === "") && typeof Plasmoid !== "undefined" && Plasmoid && Plasmoid.configuration && Plasmoid.configuration.favoriteCoins) {
            coins = Plasmoid.configuration.favoriteCoins;
            root.cfg_favoriteCoins = coins;
        }
        CoinSettingsHelper.loadFavoritesModel(coins, favoritesModel, Database);
    ...
    function serialize() {
        if (root.isUpdating) return;
        root.isUpdating = true;
        var str = CoinSettingsHelper.serializeFavorites(favoritesModel);
        root.cfg_favoriteCoins = str;
        if (typeof Plasmoid !== "undefined" && Plasmoid && Plasmoid.configuration) {
            Plasmoid.configuration.favoriteCoins = str;
        }
        root.isUpdating = false;
    }
