...
Item {
    ...
    function getCombinedFetchIds() {
        var map = {};
        if (networkManager.favoriteCoins && networkManager.favoriteCoins.trim() !== "") {
            var favs = networkManager.favoriteCoins.split(",").map(function(s) { return s.trim().toLowerCase(); }).filter(function(s) { return s !== ""; });
            for (var i = 0; i < favs.length; i++) {
                map[favs[i]] = true;
            }
        }

        var pData = PortfolioEngine.parsePortfolio(networkManager.portfolioJson);
        if (pData.enabled && Array.isArray(pData.items)) {
            for (var j = 0; j < pData.items.length; j++) {
                if (pData.items[j] && pData.items[j].id) {
                    map[pData.items[j].id.trim().toLowerCase()] = true;
                }
            }
        }
    ...
    function refreshAllData(force) {
        ...
        var now = Date.now();
        var fetchIds = networkManager.getCombinedFetchIds();
